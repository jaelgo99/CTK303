var canvas = document.getElementById("canvas90");
...
float getWaveGlow(vec2 pos, float radius, float intensity, float speed, float amplitude, float frequency, float shift){
  float dist = abs(pos.x + amplitude * sin(shift + speed * time + pos.y * frequency));
  dist = 1.0/dist;
  dist *= radius;
...
void main(){
    ...
    dist = getWaveGlow(pos, radius,intensity, 2.0, 5, 3.7, 0.0);
    col += dist * (vec3(0.1) + 0.5 + 0.5*cos(3.14+time+vec3(0,2,4)));
    
    dist = getWaveGlow(pos, radius, intensity, 4.0, 5, 6.0, 2.0);
    col += dist * (vec3(0.1) + 0.5 + 0.5*cos(1.57+time+vec3(0,2,4)));
    
    dist = getWaveGlow(pos, radius*0.5, intensity, -5.0, 5, 4.0, 1.0);
